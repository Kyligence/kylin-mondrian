...
public abstract class MondrianPropertiesBase extends TriggerableProperties {
    ...
    //use mondrian.properties under $MDX_CONF first
    static String getPropertiesPath() {
        String mdxHome = System.getProperty("MDX_CONF");
        if (!StringUtils.isEmpty(mdxHome)) {
            return mdxHome + File.separator + mondrianDotProperties;
        }

        String kyHome = System.getenv("MDX_CONF");
        if (!StringUtils.isEmpty(kyHome)) {
            String path = kyHome + File.separator + mondrianDotProperties;
            return path;
        }
