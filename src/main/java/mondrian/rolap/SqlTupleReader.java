...
public class SqlTupleReader implements TupleReader {
    ...
    int maxRows = 0;
    private Map<RolapCubeDimension, List<RolapMeasureGroup>> mapDimToMG;
    private boolean isMultipleCube;
    ...
    private void initDimAndMeasureGroupRel(TupleConstraint constraint) {
                ...
                for (int i = 0; i <= levelDepth; i++) {
                    ...
                    if (parentChild) {
                             ...
                             j++)
                        {
                            ...
                            Comparable value =
                                accessors.get(parentOrdinal + layoutOffset).get();
                            if (value == null) {
                    ...
                    for (int j = 0; j < layout.getKeys().size(); j++) {
                        ...
                        Comparable value = accessors.get(keyOrdinal + layoutOffset).get();
                    ...
                    if (member == null) {
                        ...
                        if (member == null) {
                            ...
                            if (layout.getCaptionKey() >= 0) {
                                captionValue =
                                    accessors.get(layout.getCaptionKey() + layoutOffset).get();
                            } else {
                                captionValue = null;
                            }
                            ...
                            if (layout.getNameKey() >= 0) {
                                nameObject =
                                    accessors.get(layout.getNameKey() + layoutOffset).get();
                                nameValue =
                                    nameObject == null
    ...
    Pair<String, List<SqlStatement.Type>> makeLevelMembersSql(Dialect dialect) {
        ...
        if (crossJoinParts != null && crossJoinParts.size() != 0) {
            return makeMultipleCubeLevelsSql(crossJoinParts);
        }
        return createCubeLevelsSql(dialect, measureGroupList, targets);
    }

    private Pair<String, List<SqlStatement.Type>> createCubeLevelsSql(Dialect dialect, List<RolapMeasureGroup> measureGroupList, List<Target> targets) {
...
//                  measureInCurrentbaseCube);
                    ...
                    Pair<String, List<SqlStatement.Type>> pair =
                            generateSelectForLevels(
                                    dialect,
                                    measureGroup, i, joiningMeasureGroupList.size(), targets);
                ...
                if (MondrianProperties.instance().CustomizeSql.get()) {
                    ...
                    int colCounts = 0;
                    for (Target target : targets) {
                        colCounts += target.level.getDepth();
                    }
                    ...
                    for (int i = 0; i < colCounts; i++) {
                        ...
                        preBuff.append("\"c").append(i).append("\"");
                    }
                    preBuff.append( " from ( ");
...
//            constraint.getEvaluator().setContext(originalMeasure);
            ...
            case 1:
                return generateSelectForLevels(
                        dialect, measureGroupList.get(0), 0, 1, targets);

            case 0:
                return generateSelectForLevels(dialect, null, 0, 1, targets);
        }
    }

    private Pair<String, List<SqlStatement.Type>> makeMultipleCubeLevelsSql(List<CrossJoinPart> crossJoinParts) {
        List<SqlStatement.Type> types = new ArrayList<>();
        StringBuilder buff = new StringBuilder();
        buff.append("select * from ");
        for (CrossJoinPart crossJoinPart : crossJoinParts) {
            Pair<String, List<SqlStatement.Type>> partialPair = createCrossjoinPartSql(crossJoinPart);
    ...
    private Pair<String, List<SqlStatement.Type>> createCrossjoinPartSql(CrossJoinPart crossJoinPart) {
        return createCubeLevelsSql(crossJoinPart.dialect, crossJoinPart.measureGroups, crossJoinPart.targets);
    }
            ...
            for (Target target : targets) {
                if (target.level.getDepth() != 0) {
                    ...
                    if (crossJoinPart.measureGroups == null) {
                        ...
                        layoutOffset += crossJoinPart.targets.size();
                    ...
                    target.setLayoutOffset(layoutOffset);
                }
            }
        ...
        List<Target> partialTargets)
    {
        ...
        if (queryBuilder.sqlQuery instanceof CustomizeSqlQuery) {
            ...
            return queryBuilder.toSqlAndTypes(factAndPath, measureGroup.getStar().getManyToManyPk());
        } else {
