...
//            this.resultSet = statement.executeQuery(sql);
            ...
            if (context != null) {
                this.resultSet = getResultFromSqlCache(statement, context);
            } else {
                this.resultSet = statement.executeQuery(sql);
            }
    ...
    private ResultSet getResultFromSqlCache(Statement statement, XmlaRequestContext context) throws SQLException {
    ...
    private Accessor createAccessor(int column, Type type, boolean caching) {
        ...
        case DATE:
            return new Accessor() {
                public Comparable get() throws SQLException {
                    final Date val = resultSet.getDate(columnPlusOne, Util.CalenderWithJdbcTimeZone);
        ...
        case TIMESTAMP:
            return new Accessor() {
                public Comparable get() throws SQLException {
                    final Timestamp val =  resultSet.getTimestamp(columnPlusOne, Util.CalenderWithJdbcTimeZone);
                    if (resultSet.wasNull()) {
                        return null;
                    }
                    return val;
    ...
    public enum Type {
        ...
        public Object get(ResultSet resultSet, int column) throws SQLException {
            ...
            case DATE:
                return resultSet.getDate(column + 1, Util.CalenderWithJdbcTimeZone);
            case BOOLEAN:
                return resultSet.getBoolean(column + 1);
            case TIMESTAMP:
                return resultSet.getTimestamp(column + 1, Util.CalenderWithJdbcTimeZone);
            default:
