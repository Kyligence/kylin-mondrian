...
public class FastBatchingCellReader implements CellReader {
    ...
    public Object get(RolapEvaluator evaluator) {
        // first, enable this
        boolean enableExcludeDims = true;
        final CellRequest request =
            RolapAggregationManager.makeRequest(evaluator, enableExcludeDims);
        Object result = getCellValue(request);
        if (result != Util.nullValue && request.hasExcludedDims) {
            enableExcludeDims = false;
            final CellRequest request1 = RolapAggregationManager.makeRequest(evaluator, enableExcludeDims);
            Object result1 = getCellValue(request1);
            return result1 == Util.nullValue ? result1 : result;
        }
        return result;
    }
