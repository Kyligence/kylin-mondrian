                 ...
                 axisIndex++, columnIndex++)
            {
                ...
                case DATE:
                    final Date dateValue =
                            rawRows.getDate(columnIndex + 1, Util.CalenderWithJdbcTimeZone);
                ...
                case BOOLEAN:
                    ...
                    if (booleanValue == null && rawRows.wasNull()) {
                        if (!groupingSetsList.useGroupingSets()
                                || !isAggregateNull(
                                rawRows,
                                groupingColumnStartIndex,
                ...
                case TIMESTAMP:
                    final Timestamp timestampValue =
                            rawRows.getTimestamp(columnIndex + 1, Util.CalenderWithJdbcTimeZone);
    ...
    protected static class RowList {
        ...
        static class DateColumn extends NativeColumn {
            ...
            public void populateFrom(int row, ResultSet resultSet)
                    throws SQLException
            {
                Date d = dates[row] = resultSet.getDate(ordinal + 1, Util.CalenderWithJdbcTimeZone);
                if (d == null) {
                    getNullIndicators().set(row, resultSet.wasNull());
                }
            }
        ...
        static class TimestampColumn extends NativeColumn {
            ...
            public void populateFrom(int row, ResultSet resultSet)
                    throws SQLException
            {
                Timestamp t = timestamps[row] = resultSet.getTimestamp(ordinal + 1, Util.CalenderWithJdbcTimeZone);
                if (t == null) {
                    getNullIndicators().set(row, resultSet.wasNull());
                }
            }
